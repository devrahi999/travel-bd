...
class ProfileScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: profileAsync.when(
        ...
        data: (profile) {
          final fullName = profile?['full_name'] as String? ?? (isEnglish ? 'Traveler' : 'ভ্রমণকারী');
          final email = profile?['email'] as String? ?? 'user@email.com';
                                  ...
                                  children: [
                                    ...
                                    Expanded(
                                      child: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          Text(fullName, style: AppTextStyles.headlineSm.copyWith(color: Colors.white)),
                                          Text(email, style: AppTextStyles.labelMd.copyWith(color: Colors.white70)),
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () => _showEditProfileModal(context, ref, fullName, isEnglish),
                                    ),
                                  ],
                                ),
                              ],
              ...
              SliverPadding(
                ...
                sliver: SliverList(
                  delegate: SliverChildListDelegate([
                    ...
                    // Switch to Business (Conditional)
                    if (isBusiness) ...[
                      GestureDetector(
  ...
  void _showEditProfileModal(BuildContext context, WidgetRef ref, String currentName, bool isEnglish) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(isEnglish ? 'Edit Profile' : 'প্রোফাইল সম্পাদনা', style: AppTextStyles.titleLg),
            const SizedBox(height: 16),
            TextField(
              controller: nameController,
              decoration: InputDecoration(
                labelText: isEnglish ? 'Full Name' : 'পুরো নাম',
                ...
                onPressed: () async {
                  ...
                  try {
                    ...
                    if (ctx.mounted) {
                      ...
                      Navigator.pop(ctx);
                      AppToast.show(context, isEnglish ? 'Profile updated successfully!' : 'প্রোফাইল সফলভাবে আপডেট হয়েছে!');
                    }
                  } catch (e) {
                    if (ctx.mounted) {
                      AppToast.show(context, isEnglish ? 'Failed to update profile' : 'প্রোফাইল আপডেট ব্যর্থ হয়েছে', isError: true);
                    }
                  }
                },
                style: FilledButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16)),
                child: Text(isEnglish ? 'Save Changes' : 'পরিবর্তন সংরক্ষণ করুন'),
              ),
            ),
            const SizedBox(height: 24),
          ],
